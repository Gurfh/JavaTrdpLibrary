...
- **`network`** — `UdpTransport` (`DatagramChannel`-based with multicast group management via `channel.join()`, unicast TTL via reflection into `sun.nio.ch.Net.setIntOption0()`, QoS socket options), `TcpTransport` (client/server with connection pooling, bind address/traffic class support).
